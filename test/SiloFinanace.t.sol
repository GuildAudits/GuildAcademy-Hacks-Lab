...
// contract SiloFinanceExploit is Test {
//     // Balance tracking
...
//         uint256 victimBalance = address(weth).balanceOf(victimAddress);
        
//         if (victimBalance > 0) {
//             // Impersonate victim and transfer their funds
//             vm.startPrank(victimAddress);
//             address(weth).transfer(address(this), victimBalance);
//             vm.stopPrank();
...
//         uint256 attackerBalance = address(weth).balanceOf(address(this));
//         uint256 victimBalance = address(weth).balanceOf(victimAddress);
...
//         uint256 stolenAmount = address(weth).balanceOf(address(this));
        
//         if (stolenAmount > 0) {
//             // Convert WETH to ETH
//             address(weth).withdraw(stolenAmount);
